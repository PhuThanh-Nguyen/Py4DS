...
def linearSVCModel(X_train, y_train, X_test, y_test):
	clf = LinearSVC(random_state = 645).fit(X_train, y_train)
...
def MLPClassifierModel(X_train, y_train, X_test, y_test):
	clf = MLPClassifier(random_state = 645, max_iter = 1000, early_stopping = True).fit(X_train, y_train)
	y_pred = clf.predict(X_test)
	return accuracy_score(y_test, y_pred)
...
def main():
	...
	X, y = data_train.drop('Survived', axis = 'columns'), data_train['Survived']
